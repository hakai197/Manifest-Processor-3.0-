...
public class JdbcOrderNumberDao implements OrderNumberDao {
    private static final String ORDER_SELECT = "SELECT o.order_id, o.order_number, o.customer_id, " +
            "o.shipper_id, o.trailer_id, o.door_number, o.handling_unit, o.weight, o.status, " +
            "c.name AS customer_name, s.name AS shipper_name, t.trailer_number " +
            "FROM Order_Number o " +
            "JOIN customer c ON o.customer_id = c.customer_id " +
            "JOIN shipper s ON o.shipper_id = s.shipper_id " +
            "LEFT JOIN trailer t ON o.trailer_id = t.trailer_id ";
    ...
    public List<OrderNumber> getOrdersByCustomerName(String customerName) {
        List<OrderNumber> orders = new ArrayList<>();
        String sql = ORDER_SELECT + "WHERE LOWER(c.name) = LOWER(?)";
        try {
    ...
    public List<OrderNumber> getOrdersByTrailerNumber(String trailerNumber) {
        List<OrderNumber> orders = new ArrayList<>();
        String sql = ORDER_SELECT + "WHERE t.trailer_number = ?";
        try {
    ...
    public List<OrderNumber> getOrdersByShipperName(String shipperName) {
        List<OrderNumber> orders = new ArrayList<>();
        String sql = ORDER_SELECT + "WHERE LOWER(s.name) = LOWER(?)";
        try {
    ...
    private OrderNumber mapRowToOrderNumber(SqlRowSet results) {
        ...
        orderNumber.setCustomerName(results.getString("customer_name"));
        orderNumber.setShipperName(results.getString("shipper_name"));
        orderNumber.setTrailerNumber(results.getString("trailer_number"));
