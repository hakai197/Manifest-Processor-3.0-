...
public class JdbcUnloaderDao implements UnloaderDao {
    ...
    @Override
    public List<Unloader> searchUnloaders(String query) {
        List<Unloader> unloaders = new ArrayList<>();
        String sql = "SELECT * FROM unloader WHERE LOWER(name) LIKE LOWER(?) OR employee_number LIKE ?";
        try {
            SqlRowSet results = jdbcTemplate.queryForRowSet(sql,
                    "%" + query + "%",
                    "%" + query + "%");
            ...
            throw new DaoException("Error searching unloaders", e);
        }
        return unloaders;
    }
}
