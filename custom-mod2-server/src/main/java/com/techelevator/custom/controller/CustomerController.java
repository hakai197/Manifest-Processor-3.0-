...
public class CustomerController {
    ...
    public List<Customer> searchCustomers(@RequestParam String query) {
        try {
            List<Customer> results = customerDao.searchCustomersByName(query);
            if (results.isEmpty()) {
                return results; // Return empty list instead of 404
            }
            return results;
        } catch (DaoException e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Error searching customers");
        }
    }
}